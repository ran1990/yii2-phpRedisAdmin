    ...
    if (e.currentTarget.className.indexOf('batch_del') !== -1){
      ...
      if (confirm('Are you sure you want to delete all selected keys?')) {
        $.ajax({
          type: "POST",
          url: this.href,
          data: 'post=1&selected_keys=' + selected_keys + '&'+csrfName+'=' + phpRedisAdmin_csrfToken,
      ...
      if ((e.currentTarget.href.indexOf('?') == -1) ||
        ...
        href = domain.replace('$!', 'overview');
      } else {
        href = e.currentTarget.href.substr(e.currentTarget.href.indexOf('?') + 1);
        if (href.indexOf('&') != -1) {
          href = domain.replace('$1', href).replace('&', '?');
        } else {
          href = domain.replace('$!', href);
        }
      }

      if (href.indexOf('flush') >= 0) {
